...
//   Row-level facts so we can pivot on any combination of dims.
// ============================================================
const MONTHS = ["Apr'24","May'24","Jun'24","Jul'24","Aug'24","Sep'24","Oct'24","Nov'24","Dec'24","Jan'25","Feb'25","Mar'25","Apr'25","May'25","Jun'25"];

type BProduct = "Mutual Funds" | "PMS" | "AIF Cat-II" | "AIF Cat-III" | "Direct Equity" | "Bonds / FD" | "Insurance" | "Unlisted / PE";
type BRegion = "West" | "South" | "North" | "East" | "Central";
type BSegment = "UHNI" | "HNI" | "Affluent" | "Mass Affluent";
type BChannel = "Direct RM" | "Digital Platform" | "IFA Partners" | "Family Office";
type BRm = "Anika Mehra" | "Karthik Iyer" | "Rohan Bhattacharya" | "Priya Nair" | "Vikram Sethi" | "Sneha Kapoor" | "Aditya Rao" | "Meera Joshi";

const PRODUCTS: BProduct[] = ["Mutual Funds","PMS","AIF Cat-II","AIF Cat-III","Direct Equity","Bonds / FD","Insurance","Unlisted / PE"];
const REGIONS:  BRegion[]  = ["West","South","North","East","Central"];
const SEGMENTS: BSegment[] = ["UHNI","HNI","Affluent","Mass Affluent"];
const CHANNELS: BChannel[] = ["Direct RM","Digital Platform","IFA Partners","Family Office"];
const RM_INFO: { rm: BRm; region: BRegion }[] = [
...
type BFact = {
  ...
  aum: number;       // ₹ Cr (snapshot at month-end)
  netFlows: number;  // ₹ Cr (during month)
  revenue: number;   // ₹ Cr (booked in month)
  clients: number;   // active clients touching this slice
};

const PRODUCT_WEIGHT: Record<BProduct, number> = {
...
const PRODUCT_MARGIN: Record<BProduct, number> = {
...
const REGION_WEIGHT: Record<BRegion, number> = { West: 0.33, South: 0.28, North: 0.21, East: 0.11, Central: 0.07 };
...
const TOTAL_AUM_BASE = 12_280; // ₹ Cr at latest month

const businessFacts: BFact[] = (() => {
  const r = rand(424242);
  const facts: BFact[] = [];
  MONTHS.forEach((month, mIdx) => {
    const growth = 1 + 0.012 * mIdx + (r() - 0.5) * 0.01;
    const monthTotal = TOTAL_AUM_BASE * (growth / (1 + 0.012 * (MONTHS.length - 1)));
    PRODUCTS.forEach(product => {
      REGIONS.forEach(region => {
        SEGMENTS.forEach(segment => {
          CHANNELS.forEach(channel => {
            const eligibleRms = RM_INFO.filter(x => x.region === region);
            eligibleRms.forEach(({ rm }) => {
              const w = PRODUCT_WEIGHT[product] * REGION_WEIGHT[region] * SEGMENT_WEIGHT[segment] * CHANNEL_WEIGHT[channel] / eligibleRms.length;
              const noise = 0.85 + r() * 0.3;
              const aum = +(monthTotal * w * noise).toFixed(2);
              const netFlows = +(aum * (0.008 + (r() - 0.4) * 0.012)).toFixed(3);
              const revenue = +((aum * PRODUCT_MARGIN[product] / 100) / 12).toFixed(3);
              const clients = Math.max(1, Math.round(SEGMENT_CLIENTS[segment] * REGION_WEIGHT[region] * CHANNEL_WEIGHT[channel] / eligibleRms.length * (0.6 + r() * 0.8)));
...
// ============================================================
// PORTFOLIO HOLDINGS (richer for health checks)
// ============================================================
type Holding = {
  security: string;
  issuer: string;
...
function mfH(name: string, amc: string, value: number, fee = 1.4): Holding {
  return { security: name, issuer: name, amc, product: "MF", assetClass: "Equity", sector: "Diversified", liquidity: "T+3", value, fee, underlyings: MF_UNDERLYINGS[name] ?? [] };
}
function bondMfH(name: string, amc: string, value: number, fee = 0.6): Holding {
  return { security: name, issuer: name, amc, product: "MF", assetClass: "Fixed Income", sector: "Credit", liquidity: "T+1", value, fee, underlyings: MF_UNDERLYINGS[name] ?? [] };
}
...
const clientPortfolios: ClientPortfolio[] = [
  {
    ...
    holdings: [
      mfH("HDFC Flexi Cap Fund", "HDFC AMC", 6.8),
      mfH("Mirae Asset Large Cap", "Mirae AMC", 4.2),
      bondMfH("ICICI Pru Corporate Bond", "ICICI Pru AMC", 5.1),
  ...
  },
  {
    ...
    holdings: [
      mfH("Axis Bluechip Fund", "Axis AMC", 7.8),
      mfH("SBI Small Cap Fund", "SBI AMC", 3.4, 1.9),
  ...
  },
  {
    ...
    holdings: [
      mfH("HDFC Flexi Cap Fund", "HDFC AMC", 4.6),
      mfH("Axis Bluechip Fund", "Axis AMC", 3.2),
  ...
  },
  {
    ...
    holdings: [
      mfH("Mirae Asset Large Cap", "Mirae AMC", 5.2),
      mfH("SBI Small Cap Fund", "SBI AMC", 4.4, 1.9),
  ...
  },
  {
    ...
    holdings: [
      mfH("HDFC Flexi Cap Fund", "HDFC AMC", 2.2),
      bondMfH("ICICI Pru Corporate Bond", "ICICI Pru AMC", 1.6),
  ...
  },
  {
    ...
    holdings: [
      mfH("HDFC Flexi Cap Fund", "HDFC AMC", 9.8),
      mfH("Axis Bluechip Fund", "Axis AMC", 8.4),
      bondMfH("ICICI Pru Corporate Bond", "ICICI Pru AMC", 7.2),
